...
$strHeaderTitle = '&nbsp;</b><a class="maroon" href="?vw=ACCselfBillList&mn=' . $mn . '">SENARAI</a><b>' . '&nbsp;>&nbsp;SELF BILL</b>';
...
if ($sbNo && $action == "view") {
	$sql = "SELECT a.*, b.*
			FROM   selfbill a, generalacc b
			WHERE   a.companyID = b.ID and sbNo = '" . $sbNo . "'";

	$rs 				= $conn->Execute($sql);
	$sbNo 				= $rs->fields('sbNo');

	$tarikh_selfbill 	= $rs->fields('tarikh_selfbill');
	$tarikh_selfbill 	= substr($tarikh_selfbill, 8, 2) . "/" . substr($tarikh_selfbill, 5, 2) . "/" . substr($tarikh_selfbill, 0, 4);
...
if (!isset($tarikh_selfbill)) $tarikh_selfbill = date("d/m/Y");
...
if ($perkara2) {
	...
	if ($display) print $sSQL . '<br />';
		...
		$strActivity = $_POST['Submit'] . 'Kemaskini Dokumen - ' . $sbNo;
...
if ($action == "Hapus") {
	if (count($pk) > 0) {
		...
		foreach ($pk as $val) {
			...
			$strActivity = $_POST['Submit'] . 'Hapus Kandungan Dokumen - ' . $docNo;
			activityLog($sSQL, $strActivity, get_session('Cookie_userID'), get_session('Cookie_userName'), 3);
		}
	}
	if (!$display) {
	...
	$tarikh_selfbill     = saveDateDb($tarikh_selfbill);
	$yymm           = substr($tarikh_selfbill, 0, 4) . substr($tarikh_selfbill, 5, 2);
	$sSQL   = "";
	$sWhere = "";
	$sWhere = "sbNo='" . $sbNo . "'";
	$sWhere = " WHERE (" . $sWhere . ")";
	$sSQL	= "UPDATE selfbill SET " .

		"batchNo='" . $batchNo . "'," .
		"tarikh_selfbill='" . $tarikh_selfbill . "'," .
		"companyID='" . $companyID . "'," .
		"invLhdn='" . $invLhdn . "'," .
		"invComp='" . $invComp . "'," .
		// "pymtRefer='" . $invNoRefer . "'," .
		"pymtAmt='" . $totalDb . "'," .
		"catatan='" . $catatan . "'," .
		"disedia='" . $disedia . "'," .
		"disemak='" . $disemak . "'," .
	...
	$sSQL1   = "";
	$sWhere1 = "";
	$sWhere1 = "docNo='" . $sbNo . "' AND addminus='" . 0 . "'";
	$sWhere1 = " WHERE (" . $sWhere1 . ")";
	$sSQL1	= "UPDATE transactionacc SET " .
		"pymtRefer='" . $companyID . "'," .
		// "pymtReferC='" . $invNoRefer . "'," .
		"deductID='" . $b_kodGL . "'," .
		"MdeductID='" . $Master2 . "'," .
		"batchNo='" . $batchNo . "'," .
		"desc_akaun='" . $catatan . "'," .
...
elseif ($action == "Simpan" || $simpan) {
	...
	$getMax = "SELECT MAX(CAST(right(sbNo,6) AS SIGNED INTEGER )) AS no FROM selfbill
  				WHERE sbNo LIKE 'SB%'";
	...
	if (!$display) {
		print '<script>
	window.location = "?vw=ACCselfBill&mn=' . $mn . '&action=view&add=1&sbNo=SB' . $max . '";
	</script>';
	}
}
...
$strTemp .=
	...
	. '<div>&nbsp;</div>'
	. '<div class="table-responsive"><form name="MyForm" action="?vw=ACCselfBill&mn=' . $mn . '" method="post">'
...
<tr>
	<td width="48%">
		<table border="0" cellspacing="1" cellpadding="2">
			<tr>
				<td>No. SB</td>
				<td valign="top"></td><td><input class="form-controlx"  id="sbNo" name="sbNo" value="' . $sbNo . '" type="text" size="20" maxlength="50"></td>
	...
	<td width="48%" align="right">
		<table border="0" cellspacing="1" cellpadding="2">
			<tr>
				...
				<td valign="top"></td>
				<td>
					<div class="input-group" id="tarikh_selfbill">
						<input type="text" name="tarikh_selfbill" class="form-controlx" placeholder="dd/mm/yyyy"
						data-provide="datepicker" data-date-container="#tarikh_selfbill"
						data-date-autoclose="true" value="' . $tarikh_selfbill . '">
...
<tr colspan="3">
	<td valign="top"><input name="j" type="hidden" value="tiada">
		...
		<table border="0" cellspacing="1" cellpadding="2">
			...
			<tr>
				<td>* Kod Syarikat</td><td valign="top"></td>
				<td><input name="code" value="' . $code . '" type="text" size="20" maxlength="50"  class="form-controlx" readonly/>&nbsp;';

// print '<input type="button" class="btn btn-sm btn-info" id="invButton" value="Pilih Invois" onclick="window.open(\'ACCmemberpemiutangL.php?refer=f&source=selfbill\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">&nbsp;';
print '<input type="button" class="btn btn-info btn-sm" id="compButton" value="Pilih" onclick="window.open(\'ACCidpenghutang.php?refer=f&source=selfbill\',\'sel\',\'top=10,left=10,width=950,height=500,scrollbars=yes,resizable=yes,toolbars=no,location=no,menubar=no\');">
...
print '&nbsp;<input name="loan_no" type="hidden" value=""></td>
			...
			<!-- <tr>
				...
				<td valign="top"></td>
				<td><input name="invNoRefer" value="' . $invNoRefer . '" size="20" maxlength="50"  class="form-controlx" readonly /></td>
			</tr> -->
			';

$sql3 = "SELECT * FROM transactionacc WHERE addminus IN (0) AND docNo = '" . $sbNo . "' ORDER BY ID";
...
//----------
if ($action == "view" && !is_int(dlookup("transactionacc", "ID", "docNo='" . $sbNo . "'"))) {
	print '
	<tr>
	...
	if (!$add) print '
			...
			<div class="request-loader-container" id="loaderContainer">
				<input type="button" name="add" value="Tambah" class="btn btn-sm btn-primary" onClick="window.location.href=\'?vw=ACCselfBill&mn=' . $mn . '&action=' . $action . '&sbNo=' . $sbNo . '&add=1\';">
...
<tr>
	<td>
	<input type="button" name="print" value="Cetak" class="btn btn-secondary" onClick= "print_(\'ACCinvoicedebtorPrint.php?id=' . $sbNo . '&selfbill=1\')">&nbsp;
...
document.addEventListener("DOMContentLoaded", function() {
	function compare() {
        ...
        if (!masterValue || !jumlah) {
            console.warn("Critical elements missing");
            return;
        }
